...
/* ─── صفحه ─── */
export default function BookingClient({ studioId }: { studioId: StudioId }) {
  const studioStatic = studiosData[studioId]

  type LiveStudio = { id: string; slug: string; name: string; capacity: number; pricePerHour: number; images: string[] }
        ...
        <div className="px-4 py-5 mb-28">
          ...
          <div className="grid grid-cols-2 gap-3">
            {Object.values(studiosData).filter(s => s.id !== studio.id).map((otherStatic) => {
              ...
              const otherHref = otherLive?.slug ?? otherStatic.id
              return (
              <Link key={other.id} href={`/booking/${otherHref}`} style={{ textDecoration: 'none' }}>
