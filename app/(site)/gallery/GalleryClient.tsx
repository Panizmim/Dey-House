        ...
        <div className="max-w-[860px] mx-auto px-8 py-16">
          ...
          <div className="flex flex-col" style={{ gap: 0 }}>
            {[
              ...
              'گالری دیــده جایی‌ست برای دیدن روایت‌های تازه و مواجهه با هنر معاصر؛ نه فقط به‌عنوان اثری بر دیوار، بلکه به‌عنوان بخشی از زندگی روزمره. در دیــ ده تلاش می‌کنیم مرز میان اثر هنری و زندگی روزمره را کمرنگ کنیم؛ جایی که هر‌کسی ــ بدون نیاز به پیش‌زمینه‌‌ی تخصصی ــ با هنر روبه‌رو شود و آن را نه یک رویداد استثنایی، بلکه بخشی از جریان همیشگی زندگی ببیند؛ جایی که دیدن می‌تواند شکل تازه‌ای پیدا کند. گالری د‌یـ ده بر آن‌است که نگاه‌های نو و تجربه‌های متفاوت هنری را کنار هم بیاورد و بستری باشد برای هنرمندان آوانگارد و ایده‌هایی که بتوانند با اتمسفر این فضا پیوند برقرار کنند. هدف ما ایجاد جریانی منظم و درعین‌حال صمیمی‌ست؛ تصمیمی که نه یک انتخاب فرمالیستی، بلکه بیانی از ماهیت دیــده است: هنر در مرکز، نه در حاشیه. نمایش آثار نیز در همین راستا شکل می‌گیرد؛ برای دعوت مخاطب به گفت‌وگو با هنر و هنرمند، بی‌آنکه فاصله‌ای میان او و اثر وجود داشته باشد.',
              'در دیــ ده، هنر فقط نمایش داده نمی‌شود؛ زندگی می‌کند، تغییر می‌دهد و امکان تازه‌ای برای دیدن جهان می‌سازد.',
            ].map((text, i) => (
              <p key={i} className="text-[#1a1a1a]" style={{ fontSize: 'clamp(15px, 1.8vw, 18px)', fontWeight: 400, lineHeight: 2.2, textAlign: 'justify' }}>
                {text}
              </p>
            ))}
          </div>
