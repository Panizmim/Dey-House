...
type RecentBooking = {
  ...
  user: { name: string }; studio: { name: string }
}
type RecentSubmission = { id: string; name: string; artField: string; status: string }
...
export default async function AdminPage() {
  ...
  try {
    ;[todayBookings, totalUsers, activeEvents, pendingSubs, recentBookings, recentSubmissions] =
      await Promise.all([
        ...
        db.booking.findMany({
          ...
          include: { user: { select: { name: true } }, studio: { select: { name: true } } },
        }),
        db.artistSubmission.findMany({
          take: 5, orderBy: { createdAt: 'desc' },
          select: { id: true, name: true, artField: true, status: true },
        }),
      ])
  } catch {
    dbError = true
    ...
    <div>
      ...
      {dbError && (
        <div style={{
          ...
          اتصال به دیتابیس برقرار نیست — اینترنت یا سرور Supabase را بررسی کنید.
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard title="درخواست‌های همکاری" value={pendingSubs}   icon={<Palette  size={20} />} accentColor="#F97316" />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div style={{ background: 'white', border: '1px solid #EFEFEF', borderRadius: 8, padding: 20 }}>
          ...
          {recentBookings.length === 0 ? (
            <p style={{ fontSize: 14, color: '#717171', textAlign: 'center', padding: '24px 0' }}>
              {dbError ? 'اطلاعات در دسترس نیست' : 'هیچ رزروی ثبت نشده'}
            </p>
          ) : (
        ...
        <div style={{ background: 'white', border: '1px solid #EFEFEF', borderRadius: 8, padding: 20 }}>
          ...
          {recentSubmissions.length === 0 ? (
            <p style={{ fontSize: 14, color: '#717171', textAlign: 'center', padding: '24px 0' }}>
              {dbError ? 'اطلاعات در دسترس نیست' : 'هیچ درخواستی دریافت نشده'}
            </p>
          ) : (
